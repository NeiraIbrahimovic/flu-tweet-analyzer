# Geographic Text Signal Analyzer

A Java pipeline that reads text or JSON posts, identifies flu-related text, and aggregates results by state.

## Technical depth

Layered ingestion, geographic association, classification rules, and logging.
...
Requires json-simple 1.1.1 and Java. Inspect Main for command-line arguments. Post mentions are not clinical cases and do not support disease-prevalence claims.
...
The repackaged source compiled with JDK 24. The local JUnit run passed **13 tests**, with 0 failures. Two full-dataset checks are smoke tests without assertions on the computed results. Maven configuration is provided for convenience; the reported run used javac and JUnit Console directly. Passing tests do not establish exhaustive correctness or production readiness.
